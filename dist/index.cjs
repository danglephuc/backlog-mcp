...
var TaskFileManager = class {
  ...
  async moveTaskFile(issueKey, fromPath, toPath) {
    try {
      ...
      const content = await import_fs.promises.readFile(fromPath, "utf8");
      await import_fs.promises.writeFile(toPath, content, "utf8");
      await import_fs.promises.rm(fromPath);
...
`;
  ...
  async syncIssue(issue, baseUrl, allIssues = []) {
    try {
      ...
      await import_fs.promises.writeFile(existingPath, markdownContent, "utf8");
    } catch (error) {
    ...
    try {
      ...
      for (const [issueKey, treeNode] of issueTreeMap) {
        ...
        if (parent) {
          ...
          if (!parentFolder) {
            const entries = await import_fs.promises.readdir(this.tasksDir, { withFileTypes: true });
            for (const entry of entries) {
  ...
  async saveLastSyncTime(timestamp, issues = []) {
    try {
      ...
      await import_fs.promises.writeFile(lastSyncFile, JSON.stringify(syncData, null, 2), "utf8");
    } catch (error) {
  ...
  async getLastSyncData() {
    try {
      const lastSyncFile = path.join(this.tasksDir, ".last-sync");
      const content = await import_fs.promises.readFile(lastSyncFile, "utf8");
      try {
  ...
  async syncIssues(issues, baseUrl, ignoreIssueTypes) {
    ...
    for (const issue of filteredIssues) {
      ...
      if (!existingPath) {
        ...
        const markdownContent = this.generateMarkdownContent(task);
        await import_fs.promises.writeFile(targetPath, markdownContent, "utf8");
        recoveredCount++;
  ...
  async removeTaskFile(issueKey) {
    try {
      const existingPath = await this.findExistingTaskFile(issueKey);
      if (existingPath) {
        await import_fs.promises.rm(existingPath);
      }
    } catch (error) {
  ...
  async readTaskFile(issueKey) {
    try {
      ...
      const content = await import_fs.promises.readFile(taskFilePath, "utf8");
  ...
  async findTemporaryTaskFiles(parentFolder) {
    try {
      ...
      const entries = await import_fs.promises.readdir(parentFolderPath, { withFileTypes: true });
      const tempFiles = [];
      for (const entry of entries) {
  ...
  async renameTaskFile(oldFilePath, newIssueKey) {
    try {
      ...
      await import_fs.promises.rename(oldFilePath, newFilePath);
    } catch (error) {
...
Next sync will check for updates since: ${new Date(currentSyncTime).toLocaleString()}`;
    ...
    this.server.registerTool(
      "get-issue",
      {
        ...
        inputSchema: {
          issueKey: import_zod.z.string().describe("Issue key (e.g., PROJ-123)")
        }
      },
      async ({ issueKey }) => {
        try {
...
Project: ${project.name} (${project.projectKey})`
    ...
    this.server.registerTool(
      "update-issues",
      {
        ...
        inputSchema: {
          issueKeys: import_zod.z.array(import_zod.z.string()).describe('Array of issue keys (e.g., ["PROJ-123", "PROJ-124"])')
        }
      },
      async ({ issueKeys }) => {
        try {
...
${results.join("\n")}`
  ...
  setupResources() {
    this.server.registerResource(
      "task-file",
      new import_mcp.ResourceTemplate("task://{issueKey}", { list: void 0 }),
      {
        title: "Task File",
